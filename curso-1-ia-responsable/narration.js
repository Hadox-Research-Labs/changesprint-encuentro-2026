(function () {
  ...
  function populateVoices() {
    ...
    const spanishVoices = state.voices.filter((voice) => /^es(-|_)?/i.test(voice.lang));
    const voices = spanishVoices.length ? spanishVoices : state.voices;
  ...
  function speakNextChunk(runId) {
    ...
    utterance.onerror = function (event) {
      ...
      if (selectedVoice() && !state.voiceFallback) {
        ...
        setStatus("Probando la voz predeterminada");
      ...
      setPauseLabel("Pausa");
      hideCaptions();
      setStatus(event && event.error ? `Error de narración: ${event.error}` : "Error de narración");
    };
  ...
  function speakCurrent(autoAdvance) {
    if (!("speechSynthesis" in window)) {
      setStatus("Este navegador no permite narración");
      return;
    }
  ...
  function init() {
    ...
    if ("speechSynthesis" in window) {
      ...
      setStatus("Narración no disponible");
    }
  }
